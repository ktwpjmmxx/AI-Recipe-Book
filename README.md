...
料理写真・材料・手順をまとめて保存し、人数に合わせた分量自動計算・AIアシスタントによる料理サポートを提供するフルスタックWebアプリです。企画、実装・テスト・公開デプロイ・不具合の原因調査・仕上げを実施しv5.2にて一区切りの完了としました。その後、RAG評価手法を学習する中で自作の評価スクリプトを本プロジェクトに適用し、実運用上の問題を発見・修正したv5.3を追加。さらに、LLM-as-a-judgeによるプロンプト設計の定量評価に取り組み、`search-assist`の応答生成ロジックを状況に応じて動的に切り替えるハイブリッド設計へ改善した番外編を追加しています。
...
- [アーキテクチャ](#アーキテクチャ)
- [RAG精度検証](#rag精度検証)
- [LLM-as-a-judgeによるプロンプト評価（番外編）](#llm-as-a-judgeによるプロンプト評価番外編)
- [技術的なハイライト](#技術的なハイライト)
- [ローカル起動手順](#ローカル起動手順)
...
- **LLM-as-a-judgeによる出力品質の定量評価**：プロンプトのバージョン比較を、LLM自身を審査員とした複数軸のスコアリングで実施。評価結果の傾向をもとに、状況に応じてプロンプトを動的に切り替えるハイブリッド設計へ落とし込んだ。
...
## LLM-as-a-judgeによるプロンプト評価（番外編）

`search-assist`のプロンプト設計（検索結果をどう提示し、どう回答させるか）について、LLM自身を審査員（judge）として用いた定量評価を実施した。単純な質問（現行プロンプトで十分）と、複数の類似レシピが拮抗する質問（追加指示が必要）とで最適なプロンプトが異なることを確認し、検索結果の状態に応じてプロンプトを動的に切り替えるハイブリッド設計へ改善した。

### 検証の流れ

1. 検索結果の件数・スコアの拮抗度合いをもとに8カテゴリのテストクエリを設計
2. 現行プロンプト（A）と、ノイズ無視・複数候補提示・曖昧時の確認等を指示に追加した改善版プロンプト（B）を、5軸（関連性・忠実性・網羅性・フォーマット遵守・有用性）でLLM-as-a-judgeに比較評価させた（8カテゴリ中 A: 3勝、B: 4勝）
3. 「本命が明確な質問にはA、複数候補が拮抗する質問にはB」という状況依存の傾向を確認し、検索結果のスコア差で機械的に一次分岐し、拮抗する場合のみ質問の意図をLLMに判定させて二次分岐するハイブリッドロジックを実装
4. ハイブリッド版・A単体・B単体の3者を再度比較し、ハイブリッドが最多勝利（4勝）という結果を得た

検証の過程では、改善指示の一部（DBに存在しない情報への言及回避）が本来不要な場面でも一律発動し評価を下げる副作用も発見し、質問文の内容に応じた条件付き適用へ修正した。

詳細な実験設計・実行ログ・考察は[`docs/appendix_roadmap_phase3_llm_as_judge/README.md`](docs/appendix_roadmap_phase3_llm_as_judge/README.md)を参照。評価に使用したスクリプト一式は[`backend/evaluation/scripts/`](backend/evaluation/scripts/)、実行結果の生データは[`backend/evaluation/results/`](backend/evaluation/results/)にある。
...
| バージョン | 概要 |
|---|---|
| **番外編**（LLM-as-a-judge評価） | RAG評価手法の学習の延長として、`search-assist`のプロンプト設計をLLM-as-a-judgeで定量比較。単一の正解プロンプトではなく状況依存でA/Bを動的に切り替えるハイブリッド設計へ改善 |
...
- [`docs/appendix_roadmap_phase3_llm_as_judge/README.md`](docs/appendix_roadmap_phase3_llm_as_judge/README.md) — LLM-as-a-judgeによるプロンプト評価実験の詳細レポート（番外編）
...
フルスタック開発・AI連携・認証基盤・UXデザインの実践的な学習を目的に制作した個人開発プロジェクトです。v5.2で機能実装を完了としましたが、その後RAG評価の手法（Precision@k/Recall@k/RAGAS）を学習する中で、自作した評価スクリプトを本プロジェクトに適用したところ、埋め込みモデル未設定という実運用上の問題を発見し、v5.3として修正しました。さらにLLM-as-a-judgeの手法を学習した際にも同様に本プロジェクトへ適用し、プロンプト設計を状況に応じたハイブリッド構成へ改善しています。「完成」を終着点にせず、学んだ評価手法を継続的に既存の実装へフィードバックする姿勢を大切にしています。フォーク数の表示やImagen 3による画像生成の本番有効化など、時間があれば追加で検討したいアイデアも残っています。
