...
料理写真・材料・手順をまとめて保存し、人数に合わせた分量自動計算・AIアシスタントによる料理サポートを提供します。v4.9では、機能追加ではなく**品質保証基盤の整備**（自動テスト・CI導入・DBスキーマ整合性修正）に取り組み、継続的に「壊れていないこと」を確認できる開発体制に移行しました。
...
| バージョン | 概要 |
|---|---|
| **v4.9**（本リリース） | pytestによる自動テスト29件・GitHub Actions CI・DBスキーマ整合性修正（`naming_convention`導入）を実施。品質保証基盤の整備リリース |
...
v4.7.5までは `main.py` 起動時に `_migrate_add_*()` 関数群が `ALTER TABLE` を試みる方式でマイグレーションを管理していましたが、v4.8で **Alembic** に移行しました。その過程で判明していた「`models.py`と実DBスキーマの外部キー・主キーが無名である」という課題は、v4.9で `naming_convention` の導入とbatch modeによるテーブル再作成で解消しています。詳細は[「v4.9 アップデート内容」](#v49-アップデート内容)を参照してください。
...
v4.9で `mkdir uploads`（初回のみ）とテスト実行手順が追加されました。
...
公開デモ環境の構築とアーキテクチャ図の整備、AIアシスタント（RAG）の実例整理を次のアップデートで予定しています。あわせて、v4.9で一時許容とした既存lint指摘の解消、バックエンド／フロントエンドのエラーハンドリングの体系化、APIドキュメント（Swagger）の整備にも取り組む予定です。フォーク数の表示・通知設定、Imagen 3による画像生成の本番有効化も引き続き検討しています。
