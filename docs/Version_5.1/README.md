...
料理写真・材料・手順をまとめて保存し、人数に合わせた分量自動計算・AIアシスタントによる料理サポートを提供します。v5.1では、**Gemini API認証エラーの原因調査・修正**、**バックエンドエラーレスポンスの統一**、**OpenAPIドキュメント整備**、そして**RAGライブラリ横断検索（`search-assist`）のフロントUI本実装**に取り組みました。
...
| バージョン | 概要 |
|---|---|
| **v5.1**（本リリース） | Gemini API認証エラーの原因調査・修正、バックエンドエラーレスポンス統一、OpenAPIドキュメント整備、RAG横断検索（`search-assist`）のフロントUI本実装、既存lint指摘の解消 |
| v5.0 | 公開デモ環境（Render + Vercel）構築、レシピ個別AI質問（`ai-assist`）実装、アーキテクチャ図・RAG実例ドキュメント整備 |
...
> **TL;DR:** v5.0で発見していた3つの既知課題（Gemini API認証エラー、`search-assist`のフロントUI未接続、既存lint指摘27件）にすべて対応。加えてバックエンドのエラーレスポンス形式を統一し、OpenAPI（Swagger）ドキュメントを全エンドポイントに整備した。Gemini認証エラーはコード側の2つの不整合（SDKパッケージの取り違え、Vertex AIモードへの自動切替）に加え、設定していたモデル（`gemini-2.0-flash`）自体が2026年6月1日付で廃止済みだったことが根本原因と判明。`gemini-3.5-flash`に切り替えたことで、追加コストなしで実際のAI応答（`is_mock: false`）が動作することを確認した。

<br>

### 1. Gemini API認証エラーの原因調査・修正

**背景:** v5.0で`ai-assist`検証中に発見した`401 UNAUTHENTICATED`（`ACCESS_TOKEN_TYPE_UNSUPPORTED`）エラーを調査した。

**判明した原因は2つ:**

1. **SDKパッケージの取り違え** — `requirements.txt`は旧SDK`google-generativeai==0.8.3`を固定していたが、`gemini_client.py`は新SDK`google.genai`をimportしていた。旧SDKは`google.genai`名前空間を提供しないため、`requirements.txt`通りのクリーンインストールでは本来動作しない状態だった（`ModuleNotFoundError`を実際に再現して確認）。
2. **Vertex AIモードへの自動切替** — `google-genai`のソースを確認したところ、`vertexai`引数を明示しない場合は環境変数（`GOOGLE_GENAI_USE_VERTEXAI`等）を見て自動的にVertex AIモードに切り替わる仕様だった。この場合Gemini Developer APIキーを渡しても認証形式が合わず`401`になる。
...
**追加調査で判明した本当の原因 — モデル廃止であり、billingの問題ではなかった:** 上記修正後に実キーで検証したところ、`429 RESOURCE_EXHAUSTED`（`limit: 0`）が発生し、当初は「請求先アカウント未紐付けによる無料枠制限」と判断していた。しかし別端末での最終検証時にあらためて調査したところ、設定していた`gemini-2.0-flash`が**2026年6月1日付で正式に廃止（shutdown）されていた**ことが判明した。実際に`gemini-2.5-flash`（新規ユーザー利用不可、404）→`gemini-3.5-flash`の順に切り替えて検証し、`gemini-3.5-flash`で実際にAIが生成した回答（`is_mock: false`）を確認できた。請求先アカウントの紐付けは不要だった。`config.py`の`gemini_model`のデフォルト値を`gemini-3.5-flash`に更新し、mockフォールバックは「本番の正式仕様」ではなく「LLM呼び出し失敗時の保険」という本来の位置付けに戻した。

<br>

### 2. バックエンドエラーレスポンスの統一

**設計方針 — 既存の各エンドポイントは一切変更しない:** 全routerの`raise HTTPException(status_code, "message")`の呼び出し箇所を1つも変更せず、`main.py`側の共通例外ハンドラだけで`{"error": {"code": "...", "message": "..."}}`形式に変換する構成にした。既存pytest 29件がエラーレスポンスの中身（`detail`キー等）を検証しておらず`status_code`のみの検証だったため、router側を触らずに安全に移行できると判断した。

```python
# backend/errors.py（新規）
...
`main.py`に`StarletteHTTPException` / `RequestValidationError` / 想定外の`Exception`（500）用の3つのハンドラを追加。想定外の例外は詳細をログにのみ出力し、クライアントには`INTERNAL_ERROR`のみを返すことで内部情報の漏洩を防止している。

<br>

### 3. OpenAPI（Swagger）ドキュメントの整備

全22エンドポイントに`summary`を付与し、エラーが起こりうるエンドポイントには`responses`で統一エラー形式のレスポンス例を明示した。`openapi_tags`でタグごとの説明も追加。

`POST /api/recipes/{recipe_id}/ai-assist`と`POST /api/ai/search-assist`には、「LLM呼び出し失敗時も例外を投げず常に200 OK＋`is_mock`で判別する」という設計意図を`description`に明記し、Swagger UI（`/docs`）を見ただけでAPI利用者がこの設計を理解できるようにした。

<br>

### 4. RAGライブラリ横断検索（`search-assist`）のフロントUI本実装

v5.0で意図的にスコープ外としていた機能。バックエンドAPI（`POST /api/ai/search-assist`）は実装済みだったが、フロントエンドのどの画面からも呼ばれていなかった。

**配置の設計判断:** 当初はHomePageへのバナー追加も案として検討したが、本リポジトリにおけるAI活用のメイン機能という位置づけであるため、**BottomNavの5番目のタブとして常時アクセス可能な形**を採用した。ホームの次（2番目）に配置し視認性を優先している。

**ネーミングの整理:** 既存の「検索」（レシピタイトルの単純フィルタ）と、新規のAI検索（RAGによる意味検索＋回答生成）が混同されないよう、既存機能側の表記を「レシピ内を検索…」に変更し、新機能を「AI検索」として明確に区別した（ja/en/tr 3言語対応）。

**スコア表示の設計判断:** バックエンドの`score`フィールドは「コサイン距離（小さいほど類似）」であり、そのまま`%`表示すると誤解を招くため、フロントエンド側で`関連度% = (1 - score) × 100`に変換して表示する設計とした（バックエンドのAPI仕様自体は変更していない）。

参照レシピはカード形式で表示し、`recipe_id`が存在する場合はタップでレシピ詳細へ遷移する。参照が0件の場合は空状態メッセージを表示する。

<br>

### 5. フロントエンドのAI呼び出し失敗時のtoast通知
...
外部ライブラリを追加せず、`ToastContext` + `Toast`コンポーネントを自前実装。レシピ詳細のAIチャット（`useAIPanel`）は今までモック応答でも無表示だったが、toast通知を新規追加した。`useDiscover.js`は失敗時のtoastを追加しつつ、既存の`is_mock`固定バナーは変更せず維持し、二重通知を避けている。

<br>

### 6. 既存lint指摘の解消

v4.9でCI導入時に一時許容していた27件のスタイル指摘のうち、動作に影響しない20件（`E701`/`E702`/`E712`/未使用import`F401`）を解消し、`pyproject.toml`の`ignore`リストを`E501`（日本語プロンプトの意図的な長行）のみに縮小した。

<br>

### 7. 実機に近い検証（Playwright + Chromium）

バックエンド・フロントエンドを実際に起動し、ブラウザ経由で新規登録→ログイン→ホーム表示、BottomNav 5タブのレイアウト、AI検索ページの各状態、レシピ詳細AIチャットのtoast表示、Discoverページの既存動作の回帰有無を確認した。コンソールエラーは検出されなかった。

検証中、`search-assist`の参照レシピが常に空で返る事象を発見したが、調査の結果、検証環境（サンドボックス）のネットワーク制限によりChromaDBの埋め込みモデル（ONNXファイル）のダウンロードが破損していたことが原因と判明した（コードの問題ではない）。その後、実際に別端末・実データ（食材が重複するように選定した5レシピ）で検証したところ、参照レシピが正しく複数件返ることを確認できた。

**実機検証で新たに発見・修正したバグ:** AI検索ページで、画面下部に常時表示される「レシピを追加」FABボタンが参照レシピカードと重なって表示され、関連度%が読めなくなる不具合を発見した。`BottomNav.jsx`は全ページ共通でこのFABを描画する設計だったため、AI検索ページ（`/ai-search`）でのみ非表示にする条件分岐を追加して解消した。

**実機検証で新たに発見・修正した環境起因の問題（コード自体には影響しないが、セットアップ手順として重要）:**
- `requirements.txt`のコメントに含まれる装飾文字（box drawing文字）が、日本語Windows環境の`pip`でロケール依存のエンコーディング（`cp932`）として誤解釈され、`UnicodeDecodeError`でインストール自体が失敗する事象を発見。ファイル先頭に`# -*- coding: utf-8 -*-`宣言を追加して解消（`requirements-dev.txt`も同様）
- クリーンインストール環境で`google-genai==2.11.0`が要求する`pydantic>=2.12.5`と、固定していた`pydantic==2.9.2`が競合してインストールに失敗する事象を発見。`pydantic==2.13.4`に引き上げて解消
- 同様に`requirements-dev.txt`の`httpx==0.27.2`固定が、`google-genai`が要求する`httpx>=0.28.1`と競合する事象を発見。`httpx==0.28.1`に引き上げて解消
- 設定していた`gemini_model`（`gemini-2.0-flash`）が2026年6月1日付で廃止済みだったことが判明。`gemini-3.5-flash`に更新し、実際にAIが生成した回答（`is_mock: false`）が返ることを確認した

<br>

### 8. スクリーンショット

| AI検索画面（回答＋参照レシピ3件） | BottomNav（5タブ） |
|---|---|
| ![AI検索](screenshots/05_ai_search_v51.png) | ![BottomNav](screenshots/06_bottomnav_v51.png) |

| AIチャットのモック通知（toast） | Swagger UI（統一エラー形式） |
|---|---|
...
<details>
<summary><strong>変更ファイル一覧（v5.0 → v5.1）</strong>（クリックで展開）</summary>


| ファイル | 変更内容 |
|---|---|
| `backend/errors.py`（新規） | 統一エラーレスポンスのヘルパー |
| `backend/main.py` | エラーハンドラ3種、`openapi_tags`を追加。バージョン表記を`5.1.0`に更新 |
| `backend/config.py` | `gemini_model`のデフォルト値を、廃止済みの`gemini-2.0-flash`から`gemini-3.5-flash`に更新（実機検証で発見） |
| `backend/pyproject.toml` | lint `ignore`リストを`E501`のみに縮小 |
| `backend/requirements.txt` | `google-generativeai==0.8.3` → `google-genai==2.11.0`。あわせて`pydantic`を`2.13.4`に引き上げ（依存競合解消）、UTF-8エンコーディング宣言を追加（実機検証で発見） |
| `backend/requirements-dev.txt` | `httpx`を`0.28.1`に引き上げ（依存競合解消）、UTF-8エンコーディング宣言を追加（実機検証で発見） |
| `backend/repositories/recipe_repository.py` | `== True`比較を解消（E712） |
| `backend/routers/ai.py` | OpenAPI `summary`/`description`/`responses`を追加 |
| `backend/routers/auth.py` | lint解消（E702）、OpenAPI docs追加 |
| `backend/routers/misc.py` | OpenAPI docs追加 |
| `backend/routers/public.py` | OpenAPI docs追加 |
| `backend/routers/recipes.py` | lint解消（E701/F401/E702）、OpenAPI docs追加 |
| `backend/routers/shopping_lists.py` | lint解消（E701）、OpenAPI docs追加 |
| `backend/services/ai/gemini_client.py` | lint解消（F401）、`vertexai=False`を明示 |
| `backend/services/ai/openai_client.py` | lint解消（E701） |
| `frontend/src/App.jsx` | `ToastProvider`追加、`/ai-search`ルート追加 |
| `frontend/src/api/recipeApi.js` | `getApiErrorMessage()`を追加 |
| `frontend/src/components/BottomNav.jsx` | 「AI検索」タブを追加（5タブ構成）。実機検証でAI検索ページとFAB（レシピを追加ボタン）が重なる不具合を発見し、同ページでのみFABを非表示化 |
| `frontend/src/components/Toast.jsx`（新規） | トースト表示コンポーネント |
| `frontend/src/context/ToastContext.jsx`（新規） | トースト通知のContext |
| `frontend/src/hooks/useAISearch.js`（新規） | search-assist呼び出しロジック |
| `frontend/src/hooks/useDiscover.js` | 失敗時のtoast追加 |
| `frontend/src/hooks/useRecipeDetail.js` | `useAIPanel`に`is_mock`検知とtoastを追加 |
| `frontend/src/i18n/locales/{ja,en,tr}.json` | `aiSearch`名前空間を新規追加。既存検索の表記整理 |
| `frontend/src/pages/AISearchPage.jsx`（新規） | AI検索画面 |
| `docs/Version_5.0/README.md`（新規） | v5.0時点のREADME全文をスナップショットとして退避 |
| `docs/PHASE3_SUMMARY.md`（新規） | Phase 3の意思決定記録 |
| `screenshots/05_ai_search_v51.png` 〜 `08_swagger_ui_v51.png`（新規、ルート直下） | v5.1のデモスクリーンショット4枚。次バージョン発表時に`docs/Version_5.1/`へ`README.md`とセットで退避される想定 |
...
<details>
<summary><h2 style="display:inline;">v5.0 アップデート内容（クリックで展開）</h2></summary>
...
v4.9でCIにruffを導入した際、既存コードに27件のスタイル上の軽微な指摘（動作に影響のないもの）が見つかりました。無関係な範囲まで一括で書き換えるとレビューの見通しが悪くなるため、`backend/pyproject.toml`の`ignore` / `per-file-ignores`で意図的に一時許容し、新規追加コードのみ厳格にチェックする方針としていました。

> **v5.1で対応完了。** 動作に影響しない20件（`E701`/`E702`/`E712`/未使用import）を解消し、`ignore`リストは日本語プロンプトの意図的な長行のみを対象とする`E501`のみに縮小しました。

### Gemini API認証エラーの原因切り分けについて

v5.0で発見した`401 ACCESS_TOKEN_TYPE_UNSUPPORTED`エラーは、コードレベルの原因（SDKパッケージの取り違え、Vertex AIモードへの自動切替）を先に特定・修正しましたが、その後も`429 RESOURCE_EXHAUSTED`（`limit: 0`）が残りました。当初は「請求先アカウント未紐付けによる無料枠制限」と推測しましたが、これは誤りでした。実際には設定していた`gemini-2.0-flash`が2026年6月1日付で廃止済みのモデルだったことが根本原因で、`gemini-3.5-flash`への切り替えのみで解決しました。この経験から得た教訓は、**「動かない」というエラーに対して仮説を立てるときは、外部サービスのバージョン・ライフサイクル情報を一次情報（公式ドキュメントの変更履歴）で確認してから結論を出すべき**という点です。もっともらしい仮説（billingの問題）を先に立ててしまうと、そこで調査が止まりやすくなります。

### バックエンドエラーレスポンスを統一する際、既存コードを変更しなかった理由

エラーレスポンス形式を統一するにあたり、全routerの`raise HTTPException(...)`呼び出し箇所を書き換える案と、`main.py`側の共通ハンドラだけで変換する案を比較検討しました。既存pytestがレスポンスの中身ではなく`status_code`のみを検証していたことを確認できたため、後者（router側は無変更）を選択し、変更範囲を`main.py`と新規`errors.py`の2ファイルのみに抑えています。

### RAG横断検索（`search-assist`）のUI配置について

v5.0時点では「本格的なUI/UXを設計してから導入する」とだけ方針を決めていましたが、v5.1で実装する際に改めて配置を検討し、HomePageへのバナー追加案とBottomNavタブ追加案を比較しました。本リポジトリにおけるAI活用のメイン機能という位置づけであることから、視認性を優先しBottomNavの5番目のタブ（ホームの次）として実装しています。あわせて、既存の「検索」（レシピタイトルの単純フィルタ）と新機能が混同されないよう、既存機能側の表記を「レシピ内を検索…」に変更しています。
...
**`docs/`ディレクトリのドキュメント重複**

フェーズ2で作成した`architecture.md`・`demo_examples.md`・`deployment.md`が、`docs/Version_4.9/phase1/`と`info/`の2箇所に重複して存在しています（後者の方が内容が新しい）。次回アップデートで正式なバージョンフォルダに整理する予定です。
...
v5.1での変更はありません（バックエンド／フロントエンドの起動手順自体はv4.9から変わらず）。
...
次回は本アプリの開発を総括する最終アップデートとして、`docs/Version_4.9/phase1/`と`info/`に重複しているドキュメントの整理、およびREADME全体の統合を予定しています。あわせて、フォーク数の表示・通知設定、Imagen 3による画像生成の本番有効化についても検討します。
