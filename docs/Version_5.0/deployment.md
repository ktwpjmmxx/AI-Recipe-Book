...
- `ai-assist`の動作検証中、Gemini API呼び出しが`401 UNAUTHENTICATED`(`ACCESS_TOKEN_TYPE_UNSUPPORTED`)で失敗する事象を確認した。既存のmockフォールバック機構により機能自体は継続提供されるため公開デモの運用は妨げないが、`GEMINI_API_KEY`を設定して実際のAI生成を見せたい場合は、Google Cloud Console側のAPIキー種別・認証方式を事前に確認しておく必要がある(詳細調査はPhase 3以降)
